...
# Today's Objectives
  ...
  3. Introducing R Markdown for typing/formatting reports.
...
Now that we have some basics of how R works, let's start looking at what real data looks like. A `data.frame` is the basic 2-dimensional table of data that R expects to see and analyze. R includes a sample data as `iris` for information about iris flowers.
...
Let's get some more information about our dataset, `iris`. 
```{r}
dim(iris) # Get the size of Iris
